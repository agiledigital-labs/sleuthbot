...
export const handler = async (event: SQSEvent) => {
  ...
  const bodies = event.Records.map(({body}) => JSON.parse(JSON.parse(body).Message)  as IncomingSqsMessage)
  ...
  const sendMessage = (incomingMessage: IncomingSqsMessage) => app.client.chat.postMessage({
  ...
  const results = await Promise.all(messagesOutgoing)
    ...
    .finally(() => console.log("success in sending all messages: " + new Date().toDateString()));
